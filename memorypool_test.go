...
func TestMemoryPoolReserve(t *testing.T) {
	...
	mem.CopyFrom(unsafe.Pointer(&data[0]), 10*4)
	...
	{
		half1 := mem.Slice(0, 5*4)
		half2 := mem.Slice(5*4, 5*4)
	...
	mem2 := memPool.Reserve(10 * 4)
	mem2.CopyFrom(unsafe.Pointer(&data[10]), 10*4)
	...
	mem3.CopyFrom(unsafe.Pointer(&data[20]), 5*4)
	mem4.CopyFrom(unsafe.Pointer(&data[25]), 5*4)
